...
def load_model_and_tokenizer(cfg: dict[str, Any]):
    ...
    try:
        ...
        # Gemma E2B can expose wrapper modules (e.g. Gemma4ClippableLinear) around
        # the actual linear layers. Retry LoRA on the inner `.linear` modules.
        if "not supported" not in str(exc):
            raise
        targets = list(cfg.get("lora_target_modules", []))
        wrapped_targets = [f"{t}.linear" for t in targets]
        lora_config = LoraConfig(
            r=int(cfg.get("lora_r", 32)),
            lora_alpha=int(cfg.get("lora_alpha", 64)),
