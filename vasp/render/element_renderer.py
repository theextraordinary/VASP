...
def _effective_visual_type(element: dict) -> str:
    """
    Resolve visual type robustly from source path so GIFs always use animated path.
    """
    src = str(element.get("source_uri", "")).strip().lower()
...
def _render_multi_elements(
    ...
    *,
    ...
    image_elements = [e for e in elements if _effective_visual_type(e) in ("image", "gif", "video")]
    ...
    for _, element in ordered_images:
        ...
        vtype = _effective_visual_type(element)
        if vtype == "image":
            cmd += ["-loop", "1", "-t", str(duration), "-i", str(media_path)]
        elif vtype == "gif":
    ...
    for element, input_index in image_input_map:
        ...
        for action in actions:
            ...
            if vtype == "gif":
                # Force animated cadence and fresh PTS so GIFs/stickers never collapse to static frames.
                chain = (
                    f"[{input_index}:v]"
                    f"fps=15,trim=start={trim_in}:duration={seg_dur},setpts=N/(15*TB),format=rgba"
                )
