...
from vasp.a2v.v2.creativity_policy import get_creativity_policy
...
def build_segmented_refiner_prompts(
    *,
    ...
    element_context_path: str | Path | None = None,
    ...
    element_context_text = ""
    if element_context_path and Path(element_context_path).exists():
        element_context_text = Path(element_context_path).read_text(encoding="utf-8", errors="ignore")
    ...
    asset_understanding_path = out_root.parent / "asset_understanding.txt"
    _write_asset_understanding_text(asset_understanding, asset_understanding_path)
    ...
    grouped_lookup = _build_grouped_lookup(caption_maps.get("grouped_caption_map") or [])
    element_data_map = _build_element_data_map(
        media_json=media_json,
        element_context_text=element_context_text,
    )
    ...
    for i, seg in enumerate(segments, start=1):
        ...
        segment_assets = _segment_asset_context(
            seg=seg,
            planner_asset_understanding=asset_understanding,
            element_data_map=element_data_map,
        )
        segment_caption_mapping = _segment_caption_time_mappings(
            seg=seg,
            grouped_lookup=grouped_lookup,
            preferred_sync_source=str(caption_maps.get("preferred_sync_source") or "grouped_caption_map"),
        )
        prompt = (
            f"{system_prompt}\n\n"
            f"CREATIVITY POLICY:\n{creativity_policy['rules_text']}\n\n"
            f"{json.dumps(video_summary, ensure_ascii=False, indent=2)}\n\n"
            f"{json.dumps(segment_assets, ensure_ascii=False, indent=2)}\n\n"
            f"{json.dumps(segment_caption_mapping, ensure_ascii=False, indent=2)}\n\n"
        ...
        json.dumps(
            {
                ...
                "element_context_path": str(Path(element_context_path)).replace("\\", "/") if element_context_path else None,
                "asset_understanding_path": str(asset_understanding_path).replace("\\", "/"),
...
def _write_asset_understanding_text(asset_understanding: list[dict[str, Any]], output_path: Path) -> None:
    lines: list[str] = ["ASSET UNDERSTANDING"]
    for row in asset_understanding:
        ...
        if not eid:
            continue
        typ = str(row.get("type", "")).strip() or "-"
        role = str(row.get("suggested_role", "")).strip() or "-"
        about = str(row.get("represents", "")).strip() or "-"
        best = str(row.get("best_use", "")).strip() or "-"
        usef = str(row.get("usefulness", "")).strip() or "-"
        lines.append(
            f"{eid} | type: {typ} | suggested_role: {role} | represents: {about} | best_use: {best} | usefulness: {usef}"
        )
    output_path.write_text("\n".join(lines).strip() + "\n", encoding="utf-8")


def _build_grouped_lookup(grouped: list[dict[str, Any]]) -> dict[int, dict[str, Any]]:
...
def _build_element_data_map(*, media_json: dict[str, Any], element_context_text: str) -> dict[str, dict[str, Any]]:
    ...
    if element_context_text:
        for eid in list(out.keys()):
            snippet = _extract_element_snippet(element_context_text, eid)
            if snippet:
                out[eid]["element_data"] = snippet
    return out


def _extract_element_snippet(text: str, element_id: str) -> str | None:
...
def _segment_asset_context(
    *,
    seg: dict[str, Any],
    planner_asset_understanding: list[dict[str, Any]],
    element_data_map: dict[str, dict[str, Any]],
) -> list[dict[str, Any]]:
    ...
    if isinstance(vc, list):
        for row in vc:
            if isinstance(row, dict) and row.get("element_id"):
                segment_ids.add(str(row.get("element_id")).strip())
    out: list[dict[str, Any]] = []
    for row in planner_asset_understanding:
        if not isinstance(row, dict):
            continue
        eid = str(row.get("element_id", "")).strip()
        if not eid or eid not in segment_ids:
            continue
        merged = dict(row)
        if eid in element_data_map:
            merged["element_data"] = element_data_map[eid]
        out.append(merged)
    return out


def _segment_caption_time_mappings(
    *,
    seg: dict[str, Any],
    grouped_lookup: dict[int, dict[str, Any]],
    preferred_sync_source: str,
) -> dict[str, Any]:
    idxs = seg.get("caption_indices")
    out_grouped: list[dict[str, Any]] = []
    if isinstance(idxs, list):
        for i in idxs:
            if isinstance(i, int) and i in grouped_lookup:
                out_grouped.append(grouped_lookup[i])
    return {
        "word_timing_map": [],
        "grouped_caption_map": out_grouped,
        "preferred_sync_source": preferred_sync_source or "grouped_caption_map",
    }
