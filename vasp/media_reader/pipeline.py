...
def generate_input_json(
    *,
    ...
    for idx, media_item in enumerate(normalized_media, start=1):
        ...
        about_text = str(media_item.get("about") or "").strip()
        aim_text = str(media_item.get("aim") or "").strip()
        ...
        # For video media, only transcribe audio when explicitly requested by
        # the captions metadata. Accept both aim and about because captions.txt
        # files commonly put "extract captions" in either column.
        if media_type == "video" and not _should_extract_audio_from_metadata(about_text, aim_text):
            do_asr = False
        ...
        if media_type == "video":
            ...
            if do_asr:
                ...
                transcript = _annotate_asr_coverage(transcript, probe)
...
def _should_extract_audio_from_about(about: str | None) -> bool:
    return _should_extract_audio_from_metadata(about, None)


def _should_extract_audio_from_metadata(about: str | None, aim: str | None) -> bool:
    text = f"{about or ''} {aim or ''}".strip().lower()
    if not text:
        return False
    triggers = (
        "extract audio",
        ...
        "transcribe",
        "transcript",
        "main visual to extract caption",
        "main video to extract caption",
    )
...
def _annotate_asr_coverage(transcript: dict[str, Any], probe: MediaProbeInfo) -> dict[str, Any]:
    if not isinstance(transcript, dict):
        return transcript
    try:
        media_duration = float(probe.duration or 0.0)
    except Exception:
        media_duration = 0.0
    if media_duration <= 0:
        return transcript
    groups = transcript.get("caption_groups")
    words = transcript.get("word_timing_map") or transcript.get("words")
    last_end = 0.0
    if isinstance(groups, list) and groups:
        try:
            last_end = max(last_end, float(groups[-1].get("end") or 0.0))
        except Exception:
            pass
    if isinstance(words, list) and words:
        try:
            last_end = max(last_end, float(words[-1].get("end") or 0.0))
        except Exception:
            pass
    out = dict(transcript)
    ...
    out["asr_coverage"] = {
        ...
        "uncovered_tail_s": round(max(0.0, media_duration - last_end), 3),
    }
    if media_duration - last_end > 2.0:
        warnings.append(f"asr_stopped_before_media_end:last_caption_end={last_end:.3f}:media_duration={media_duration:.3f}")
